...
void onDataReceive(const esp_now_recv_info *info, const uint8_t *incomingData, int len) {
  if (len == sizeof(struct_heartbeat)) {
    ...
    completeCSV += String(receivedMessage.csv_data, receivedMessage.data_length);

    if (receivedMessage.is_last_chunk) {
      Serial.print("Received complete CSV via ESP-NOW: ");
      Serial.println(completeCSV);
      ...
      if (eth_connected && (uint32_t)ETH.localIP()) {
        ...
        if (Udp.endPacket()) {
          ...
          Serial.println("Failed to send complete CSV data via UDP");
        }
      }
      completeCSV = ""; // Reset for next transmission
    }
  }
}
