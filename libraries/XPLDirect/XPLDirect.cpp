			...
			switch (_dataRefs[i]->dataRefVARType)
			{
				case XPL_DATATYPE_INT:
				{
							...
							&& millis() - _dataRefs[i]->lastUpdateTime > _dataRefs[i]->updateRate || _dataRefs[i]->forceUpdate)
						{
							...
							_sendPacket(XPLCMD_DATAREFUPDATE, _dataRefs[i]->dataRefHandle, (int)*(long int*)_dataRefs[i]->latestValue);
				...
				case XPL_DATATYPE_FLOAT:
				{
							...
							&& (millis() - _dataRefs[i]->lastUpdateTime) > _dataRefs[i]->updateRate) || _dataRefs[i]->forceUpdate)
						{
							...
							_sendPacket(XPLCMD_DATAREFUPDATE, _dataRefs[i]->dataRefHandle, *(float*)_dataRefs[i]->latestValue);
		...
		if (_commands[i]->commandHandle >= 0 && millis() - _commands[i]->lastUpdateTime > XPL_COMMAND_MAX_UPDATE_RATE_MILLIS)
		{
			if (_commands[i]->pin >= 0)			// process if assigned pin
			{
				...
				if (pinStatus != _commands[i]->lastSentValue)
				{
					...
					if (pinStatus == true)
					{
						_sendPacket(XPLCMD_COMMANDEND, _commands[i]->commandHandle);
						_commands[i]->lastUpdateTime = millis();					// only for initial press to debounce
					}
					else
					{
						_sendPacket(XPLCMD_COMMANDSTART, _commands[i]->commandHandle);
			...
			else								// process for normal variables
			{
				...
				if (_commands[i]->latestValue != NULL && *(int *)_commands[i]->latestValue != _commands[i]->lastSentValue)
				{
					...
					if (*_commands[i]->latestValue > 0)	_sendPacket(XPLCMD_COMMANDEND, _commands[i]->commandHandle);
					else										_sendPacket(XPLCMD_COMMANDSTART, _commands[i]->commandHandle);
...
int XPLDirect::commandTrigger(int commandHandle, int triggerCount)
{
	...
	_sendPacket(XPLCMD_COMMANDTRIGGER, _commands[commandHandle]->commandHandle, triggerCount);
...
int XPLDirect::sendDebugMessage(char* msg)
{
	...
	_sendPacket(XPLCMD_PRINTDEBUG, msg);
...
void XPLDirect::_sendname()
{
	if (_deviceName != NULL)
	{
		_sendPacket(XPLRESPONSE_NAME, _deviceName);
	}
...
*/
	...
	switch (_receiveBuffer[1])
	{
		...
		{   int packetSent = 0;
			...
			{
				if (_dataRefs[i]->dataRefHandle < 0)
			...
			{
				if (_commands[i]->commandHandle < 0)
				{
					if (_commands[i]->commandName != NULL)
...
void XPLDirect::_sendPacket(int command, int handle, int value)			// for ints
{
	if (handle < 0) return;
	//sprintf(_sendBuffer, "%c%c%3.3i%07.7i%c", XPLDIRECT_PACKETHEADER, command, handle, value, XPLDIRECT_PACKETTRAILER);
	sprintf(_sendBuffer, "%c%c%3.3i%i%c", XPLDIRECT_PACKETHEADER, command, handle, value, XPLDIRECT_PACKETTRAILER);
	_transmitPacket();

}

void XPLDirect::_sendPacket(int command, int handle, float value)		// for floats
{
	if (handle < 0) return;
// some boards cant do sprintf with floats so this is a workaround.  What a pain.
...
void XPLDirect::_sendPacket(int command, int handle)			// just a command with a handle
{
	if (handle < 0) return;
...
void XPLDirect::_sendPacket(int command, char *str)			// for a string
{
...
//	streamPtr->print("writing packet: ");
//	streamPtr->println(_sendBuffer);
...
int XPLDirect::registerDataRef(char* datarefName, int rwmode, unsigned int rate, float divider, long int* value)
{
...
int XPLDirect::registerDataRef(char* datarefName, int rwmode, unsigned int rate, float divider, long int* value, int index)
{
...
int XPLDirect::registerDataRef(char* datarefName, int rwmode, unsigned int rate, float divider, float* value)
{
...
int XPLDirect::registerDataRef(char* datarefName, int rwmode, unsigned int rate, float divider, float* value, int index)
{
...
int XPLDirect::registerCommand(char* commandName, int* value)
{
...
int XPLDirect::registerCommand(char* commandName, int pin)		// associate a digital pin with an xplane command
{
...
int XPLDirect::registerCommand(char* commandName)		// user will trigger commands with commandTrigger
{
